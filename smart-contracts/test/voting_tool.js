...
contract("VotingTool", async (accounts) => {
  ...
  describe("after add a candidate", () => {
    it("should be present on the candidate list", async () => {
      ...
      assert.equal(
        candidate1,
        actual.find((element) => element === candidate1)
      );
    });
  ...
  describe("after reset data", () => {
    it("should reset votes emmiting 'DataResetByOwner' event", async () => {
      await contract.registerCandidate(candidate4);
      await contract.vote(candidate4, { from: accounts[5] });
      let receipt = await contract.resetVoting();
      let actual = receipt.logs[0].event;
      assert.equal('DataResetByOwner', actual);
    });
  });
});
